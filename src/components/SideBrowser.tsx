...
interface SideBrowserProps {
  ...
  onAddPlugin?: (type: string, metadata?: any) => void; // Deprecated prop kept for compatibility but not used
...
const SideBrowser: React.FC<SideBrowserProps> = ({ 
  ...
  // HOOK DU CONTEXTE (Nouveau)
  const { state, addPlugin } = useDAW();
  ...
  // LISTE VIDE COMME DEMANDÉ
  const AVAILABLE_FX: { id: string, name: string, desc: string, icon: string, color: string }[] = [];
  ...
  const handleTriggerPlugin = (type: string, metadata?: any) => {
      // 1. Vérifier si une piste est sélectionnée
      if (!state.selectedTrackId) {
          alert("Veuillez sélectionner une piste d'abord.");
          return;
      }
      // 2. Ajouter via le Context
      addPlugin(state.selectedTrackId, type as PluginType, metadata);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (selected) {
        if (activeTab === 'fx') handleTriggerPlugin((selected as any).id);
        else if (activeTab === 'nova') handleTriggerPlugin('VST3', selected);
      }
        ...
        {activeTab === 'fx' && (
          <div className="space-y-3 pb-20 p-4 h-full overflow-y-auto custom-scroll">
            ...
            <div className="space-y-2">
              {filteredInternalFX.length === 0 && (
                  <div className="py-10 text-center opacity-40">
                    <p className="text-[10px] font-black uppercase text-slate-500">Aucun effet disponible</p>
                  </div>
              )}
              ...
              {filteredInternalFX.map((fx, idx) => (
                <div 
                  ...
                  onClick={() => handleTriggerPlugin(fx.id)}
            ...
            <div className="space-y-1">
              {filteredNovaPlugins.map((p, idx) => (
                <div 
                  ...
                  onClick={() => handleTriggerPlugin('VST3', p)}
                  onMouseEnter={() => setSelectedIndex(idx)}
